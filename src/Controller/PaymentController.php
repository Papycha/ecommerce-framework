...
class PaymentController extends AbstractController
{
    ...
    public function payment(OrdersRepository $repository, $reference, EntityManagerInterface $em): Response
    {
    ...
    public function paymentSuccess(OrdersRepository $repository, $stripeSession, EntityManagerInterface $em, Cart $cart)
    {
        ...
        // $content = "Bonjour {$user->getUserIdentifier()} nous vous remercions de votre commande";
        // (new Mail)->send(
        //     // $user->getEmail(),
        //     // $user->getFirstname(),
    ...
    public function paymentFail(OrdersRepository $repository, $stripeSession)
    {
        $order = $repository->findOneByStripeSession($stripeSession);
        if (!$order || $order->getUser() != $this->getUser()) {
            throw $this->createNotFoundException('Commande innaccessible');
